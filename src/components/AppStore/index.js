...
class AppStore extends Component {
  ...
  onChangeTabItems = event => {
    this.setState({searchInput: event.target.value})
  }

  getFilteredTabs = AppList => {
    const {activeTabId} = this.state

    const {filteredTabs} = AppList.filter(
  ...
  getSearchedResults = () => {
    ...
    const searchResults = appsList.filter(eachAppList =>
      eachAppList.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
  ...
  render() {
    ...
    const {filteredTabs} = this.getFilteredTabs(searchResults)
    return (
        ...
        <div className="card-container">
          ...
          <div className="search-container">
            <input
              ...
              placeholder="search"
              value={searchInput}
              onChange={this.onChangeTabItems}
              className="input-box"
            />
            <img
          ...
          <ul className="app-item-list">
            {filteredTabs.map(eachAppList => (
              <AppItem appItemDetails={eachAppList} key={eachAppList.appId} />
            ))}
          </ul>
        </div>
      </div>
